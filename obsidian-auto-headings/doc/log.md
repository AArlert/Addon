...
## 2026-06-25 — 修复跳级标题少一段序号的 bug（0.3.3）

**交接人**：agent（claude/heading-numbering-fourth-level-76p04p 分支）

**用户反馈**：从「第四级标题」（即 H5）起序号少一位——H3 后直接跟 H5 时，H5 被当作 H4
（如 `5.a.a` 三段，应为 `5.a.1.a` 四段）。而当中间的 H4 真实存在时，H5 正常呈现四段。

**做了什么**：
- 根因在 `buildPrefix`（`src/numbering.ts`）：原逻辑对跳级时计数器为 0 的中间祖先**整段丢弃**
  （为避免 `1.0.1` 幻影 0），导致 H_n 的序号段数少于其深度。改为**缺失的中间祖先按该级样式的
  「1」占位呈现**——计数器是 1 基的，「第 0 节」无意义，故以 1 占位最自然，且与「无跳级」时的
  输出完全一致（跳级 H5 与正常 H5 均得 `…1.a`）。
- **关键不变量**：占位只影响拼接显示，该级计数器**本身仍保持 0**，直到真正出现该级标题才从 1
  累加。因此跳级 H5 在前、随后首个真实 H4 仍为 `…1`（不被借号），与用户实测期望一致。
- 同步修订：README 「边界情况」表中标题层级跳跃一行的描述；`numbering.test.ts` 更新原
  `H2→H4` 用例（`1.1`→`1.1.1`），并新增两个用例覆盖「H3→H5 保有四段」与「占位不借号」。
- 版本 0.3.2 → 0.3.3（manifest/package/versions），`npm run release` 重新生成 `release/`。

**没做什么**：未改动计数器状态机本身（`HeadingCounter` 的累加/归零语义不变，跳级中间级别仍
不实例化）；未触碰白名单（M4）、错位 H1 降级等其他逻辑。

**下一步**：继续按 README Roadmap 推进（白名单 M4 等）。若用户对「缺失祖先以 1 占位」的呈现
另有偏好（如希望显示真实的 0、或显式提示跳级），再据反馈调整 `buildPrefix` 的 `display` 取值。

**验证方式**：项目根 `npm test`（78 passed）、`npm run lint`、`npm run format:check` 全绿；
`npm run release` 后 `git status` 可见 `release/` 已更新。手动复现：H2/H3 后直接写 H5，
默认模板下应得 `1.1.1.1`（四段）。
