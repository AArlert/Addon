...
| `AutoHeadingsPlugin` | `src/main.ts:43` |
| `AutoHeadingsPlugin.applyRenumber` | `src/main.ts:480` |
| `AutoHeadingsPlugin.clearAllVaultNumbering` | `src/main.ts:360` |
| `AutoHeadingsPlugin.currentFileHeadings` | `src/main.ts:209` |
| `AutoHeadingsPlugin.currentFilePath` | `src/main.ts:218` |
| `AutoHeadingsPlugin.getTemplateForFile` | `src/main.ts:154` |
| `AutoHeadingsPlugin.loadSettings` | `src/main.ts:378` |
| `AutoHeadingsPlugin.messages` | `src/main.ts:58` |
| `AutoHeadingsPlugin.onload` | `src/main.ts:62` |
| `AutoHeadingsPlugin.onunload` | `src/main.ts:128` |
| `AutoHeadingsPlugin.renameTemplate` | `src/main.ts:228` |
| `AutoHeadingsPlugin.renumberActiveFile` | `src/main.ts:251` |
| `AutoHeadingsPlugin.runClearForeignNumbering` | `src/main.ts:317` |
| `AutoHeadingsPlugin.runClearNumbering` | `src/main.ts:271` |
| `AutoHeadingsPlugin.runImmediateRenumber` | `src/main.ts:447` |
| `AutoHeadingsPlugin.saveSettings` | `src/main.ts:403` |
| `AutoHeadingsPlugin.scheduleRenumber` | `src/main.ts:411` |
| `AutoHeadingsPlugin.setAutoNumber` | `src/main.ts:139` |
| `AutoHeadingsPlugin.shouldAutoTrigger` | `src/main.ts:174` |
| `AutoHeadingsPlugin.strippableAffixes` | `src/main.ts:193` |
...
| `AutoHeadingsSettingTab.deleteTemplate` | `src/settings/SettingsTab.ts:455` |
| `AutoHeadingsSettingTab.display` | `src/settings/SettingsTab.ts:99` |
| `AutoHeadingsSettingTab.previewText` | `src/settings/SettingsTab.ts:888` |
| `AutoHeadingsSettingTab.renderDangerZone` | `src/settings/SettingsTab.ts:816` |
| `AutoHeadingsSettingTab.renderEditPanel` | `src/settings/SettingsTab.ts:464` |
| `AutoHeadingsSettingTab.renderPathRuleRow` | `src/settings/SettingsTab.ts:242` |
| `AutoHeadingsSettingTab.renderPathRules` | `src/settings/SettingsTab.ts:193` |
| `AutoHeadingsSettingTab.renderTemplateRow` | `src/settings/SettingsTab.ts:399` |
| `AutoHeadingsSettingTab.renderWhitelistEditor` | `src/settings/SettingsTab.ts:698` |
| `AutoHeadingsSettingTab.requestDeleteTemplate` | `src/settings/SettingsTab.ts:445` |
| `AutoHeadingsSettingTab.saveAndPreview` | `src/settings/SettingsTab.ts:872` |
| `AutoHeadingsSettingTab.t` | `src/settings/SettingsTab.ts:95` |
| `AutoHeadingsSettingTab.textCell` | `src/settings/SettingsTab.ts:856` |
| `AutoHeadingsSettingTab.updatePathDatalist` | `src/settings/SettingsTab.ts:363` |
| `buildPrefix` | `src/numbering.ts:527` |
| `cjkSection` | `src/numbering.ts:358` |
| `clampDebounceDelay` | `src/settings.ts:56` |
| `CleanupOptions` | `src/cleanup.ts:13` |
| `clearForeignNumberingContent` | `src/cleanup.ts:66` |
| `clearNumberingContent` | `src/cleanup.ts:34` |
| `ClearVaultModal` | `src/settings/SettingsTab.ts:985` |
| `ClearVaultModal.constructor` | `src/settings/SettingsTab.ts:988` |
| `ClearVaultModal.onClose` | `src/settings/SettingsTab.ts:1012` |
| `ClearVaultModal.onOpen` | `src/settings/SettingsTab.ts:993` |
...
| `defaultPathRules` | `src/settings.ts:43` |
| `DeleteTemplateModal` | `src/settings/SettingsTab.ts:906` |
| `DeleteTemplateModal.applyAndClose` | `src/settings/SettingsTab.ts:958` |
| `DeleteTemplateModal.constructor` | `src/settings/SettingsTab.ts:913` |
| `DeleteTemplateModal.onClose` | `src/settings/SettingsTab.ts:974` |
| `DeleteTemplateModal.onOpen` | `src/settings/SettingsTab.ts:920` |
...
- L481 `export function getMessages(lang: Lang): Messages` — 取某语言的文案表。

### src/main.ts （513 行）

- L43 `class AutoHeadingsPlugin` — obsidian-auto-headings 插件入口。
- L58 `messages(): Messages` — 当前界面语言的文案表（按 `settings.language` 解析，见  / ）。
- L62 `async onload(): Promise<void>`
- L128 `onunload(): void`
- L139 `async setAutoNumber(autoNumber: boolean): Promise<void>` — 设置「全局自动编号」开关并持久化，作为面板开关与命令之间的单一数据源，确保两者双向同步。
- L154 `getTemplateForFile(filePath: string | undefined | null): Template | null` — 按路径规则解析**某文件**应使用的模板（见 spec.md §3.8）。
- L174 `private shouldAutoTrigger(content: string): boolean` — **自动触发**是否应进行（见 spec.md §3.1 自动路径）。判定顺序：
- L193 `strippableAffixes(): { prefixes: string[]; suffixes: string[] }` — 收集**全部模板各级别在用的前缀 / 后缀并集**，供剥离时识别历史前缀（方案 A，见
- L209 `currentFileHeadings(): Heading[]` — 解析**当前活动 Markdown 文件**的标题列表，供设置面板的白名单实时命中预览使用（见
- L218 `currentFilePath(): string | null` — 当前活动 Markdown 文件的路径（无活动视图时为 null），供设置面板的白名单预览取模板。
- L228 `async renameTemplate(oldName: string, newName: string): Promise<boolean>` — 重命名模板，并同步更新 `data.json` 中引用该模板名的路径规则（见 spec.md §3.6/§3.8）。
- L251 `renumberActiveFile(): void` — 在设置面板修改模板后，立即对**当前活动 Markdown 文件**重新编号，使格式调整即时可见。
- L271 `private runClearNumbering(editor: Editor, ctx: MarkdownView | MarkdownFileInfo): void` — 「清除当前文件编号」命令（**手动路径**，见 spec.md §3.10）：剥离当前文件所有标题的编号
- L317 `private runClearForeignNumbering(editor: Editor, ctx: MarkdownView | MarkdownFileInfo): void` — 「清理非本插件的标题编号」命令（**手动路径**，0.6.6，见 spec.md §3.10）：只剥**不含 WJ** 的
- L360 `async clearAllVaultNumbering(): Promise<void>` — 清除全库所有 Markdown 文件的编号前缀（见 spec.md §3.10「清除全库编号」按钮）。
- L378 `async loadSettings(): Promise<void>`
- L403 `async saveSettings(): Promise<void>`
- L411 `private scheduleRenumber(editor: Editor, info: MarkdownView | MarkdownFileInfo): void` — 实时编辑触发（**自动路径**）：达到自动触发资格才重置该文件的防抖计时器；到期后再次
- L447 `private runImmediateRenumber(editor: Editor, ctx: MarkdownView | MarkdownFileInfo): void` — 「立即重新编号」命令（**手动路径**，见 spec.md §3.1）：绕过防抖、绕过「全局自动编号」开关
- L480 `private applyRenumber(editor: Editor, template: Template): boolean` — 用给定模板对编辑器执行一次重新编号，并以**单一事务**写回变化的行。
...
### src/settings/SettingsTab.ts （1016 行）
...
- L99 `display(): void`
- L193 `private renderPathRules(containerEl: HTMLElement): void` — 渲染路径规则区（见 spec.md §3.8）：可视化表格（路径模式 → 模板），可增删、可拖拽排序、可滚动；
- L242 `private renderPathRuleRow(table: HTMLElement, rule: PathRule, index: number): void` — 渲染单条路径规则行（拖拽手柄 + 行号 + 路径输入[含清空] + 模板下拉 + 删除）。
- L363 `private updatePathDatalist(datalist: HTMLDataListElement, inputValue: string): void` — **分层**填充路径补全 `<datalist>`：仅列出当前输入所在目录的**直接子项**（输入 `/` 先给根与
- L399 `private renderTemplateRow(parent: HTMLElement, template: Template): void` — 渲染单个模板的行（标题行 + 可展开编辑面板）。
- L445 `private async requestDeleteTemplate(template: Template): Promise<void>` — 删除模板：若**未被任何路径规则引用**则直接删除；否则弹出「知情确认 + 安全降级」对话框
- L455 `async deleteTemplate(name: string): Promise<void>` — 真正执行模板删除并刷新面板（收起其编辑面板）。
- L464 `private renderEditPanel(parent: HTMLElement, template: Template, isDefault: boolean): void` — 渲染某模板的行内编辑面板：可选改名 + 起始/结束层级 + 五级×五列网格 + 实时预览。
- L698 `private renderWhitelistEditor(panel: HTMLElement, template: Template): void` — 渲染某模板的白名单编辑器（模板级配置，见 spec.md §3.7）。
- L816 `private renderDangerZone(containerEl: HTMLElement): void` — 渲染「危险区域」（**默认折叠**，见 spec.md §3.10）：点击标题展开后才显示「清除全库编号」。
- L856 `private textCell( row: HTMLElement, value: string, placeholder: string, onChange: (value: string) =…` — 创建一个文本输入单元格，封装 onChange。
- L872 `private async saveAndPreview( template: Template, level: number, key: LevelKey, previewEls: Map<Lev…` — 保存模板并刷新该级的预览文本。
- L888 `private previewText(template: Template, level: number): string` — 计算某级的预览字符串（取前三个同级序号示例）；在编号区间之外时显示「（不编号）」。
- L906 `class DeleteTemplateModal` — 删除被路径规则引用的模板时的「知情确认 + 安全降级」对话框（见 spec.md §3.6）。
- L913 `constructor(app: App, templateName: string, affected: PathRule[], tab: AutoHeadingsSettingTab)`
- L920 `onOpen(): void`
- L958 `private async applyAndClose(plugin: AutoHeadingsPlugin): Promise<void>` — 按选择改写 / 删除受影响规则，再删除模板，刷新面板并关闭。
- L974 `onClose(): void`
- L985 `class ClearVaultModal` — 「清除全库编号」二次确认对话框（见 spec.md §3.10）。
- L988 `constructor(app: App, plugin: AutoHeadingsPlugin)`
- L993 `onOpen(): void`
- L1012 `onClose(): void`
