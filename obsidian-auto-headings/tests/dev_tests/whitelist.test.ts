...
describe("D1 / D7 — exact 命中：不写前缀、不占槽位、豁免即去号", () => {
	it("D1：exact 命中不占计数器槽位（下一个 H2 不跳号）", () => {
		...
		expect(prefixes(content, tpl)).toEqual([null, "1 ", null, "2 "]);
	});

	it("D7：命中标题身上已有旧编号 → 豁免时剥掉旧编号（去号）", () => {
		const tpl = withWhitelist([{ text: "目录", match: "exact" }]);
		const out = renumberContent("## 1 目录", tpl);
		expect(out).toBe("## 目录");
	});
});
...
describe("D2 — exact 命中的归一化健壮性", () => {
	...
	it("非命中标题正常编号（## 目录说明 不等于 目录）", () => {
		expect(renumberContent("## 目录说明", tpl)).toBe("## 1 目录说明");
	});
});
...
describe("D3 — partial 部分匹配", () => {
	...
	it("## 附录 A / ## 技术附录 都命中（包含子串）", () => {
		...
		expect(prefixes(content, tpl)).toEqual([null, null, "1 "]);
	});
});
...
describe("D4 — subtree 子树匹配", () => {
	...
	it("根与整棵子树都不编号、均不占槽位（后续同级不跳号）", () => {
		...
		expect(prefixes(content, tpl)).toEqual(["1 ", null, null, null, "2 "]);
	});
...
describe("默认模板：预填充中英结构性词表自动生效", () => {
	it("摘要 / 致谢 / References / Abstract 等不被编号", () => {
		...
		expect(prefixes(content, DEFAULT_TEMPLATE)).toEqual([null, "1 ", null, "2 "]);
	});
