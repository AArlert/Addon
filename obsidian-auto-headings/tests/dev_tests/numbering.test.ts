...
describe("改前缀/后缀后再触发不叠加（方案 A，回归 testplan B2/B3）", () => {
	...
	it("B2：前缀 空→『第』，旧『1 标题』被剥、不叠成『第1 1 标题』", () => {
		...
		expect(once).toBe("## 1 标题");
		// 改前缀为「第」后再触发：空前缀写出的旧号被「空串候选」剥掉，只留正确新前缀。
		const after = mk("第", "");
		expect(renumberContent(once, after)).toBe("## 第1 标题");
		expect(renumberContent(renumberContent(once, after), after)).toBe("## 第1 标题");
	});
	...
	it("B3：后缀 空→『章』，不叠成『1章 1 标题』", () => {
		...
		expect(once).toBe("## 1 标题");
		const after = mk("", "章");
		expect(renumberContent(once, after)).toBe("## 1章 标题");
		expect(renumberContent(renumberContent(once, after), after)).toBe("## 1章 标题");
	});

	it("「第…章」式整套：父子段都不叠加、幂等", () => {
		const tpl = mk("第", "章");
		const once = renumberContent("## 父\n### 子", tpl);
		expect(once).toBe("## 第1章 父\n### 第1.1章 子");
		expect(renumberContent(once, tpl)).toBe(once);
		// 从「第…章」改回无前后缀：靠注入并集（含「第」「章」）把旧前缀剥净。
		const bare = mk("", "");
		const opts = { strippablePrefixes: ["第", ""], strippableSuffixes: ["章", ""] };
		expect(renumberContent(once, bare, opts)).toBe("## 1 父\n### 1.1 子");
	});

	it("反向 非空→空（『第』→空）：靠注入并集剥净，不叠成『1 第1 标题』", () => {
		const withPrefix = renumberContent("## 标题", mk("第", ""));
		expect(withPrefix).toBe("## 第1 标题");
		// 当前模板已无「第」，但 main.ts 传入的并集仍含「第」→ 旧前缀可被剥离。
		const opts = { strippablePrefixes: ["第", ""] };
		expect(renumberContent(withPrefix, mk("", ""), opts)).toBe("## 1 标题");
	});

	it("安全性：前缀候选只在其后紧跟序号时才成前缀，不吞普通标题正文", () => {
		// 标题以「第」起头但其后非序号（「印象」）→ 不构成前缀 → 正文完整保留，仅在左侧加新前缀。
		expect(renumberContent("## 第一印象", mk("第", ""))).toBe("## 第1 第一印象");
		// 不以「第」也不以序号起头 → 完全不受并集候选影响。
		expect(renumberContent("## API 设计", mk("第", ""))).toBe("## 第1 API 设计");
	});
});
...
describe("2024 折中：只剥一层，序号后的用户数字被保留（testplan E5）", () => {
	it("首次把以数字起头的标题编号：吃掉用户数字（既定取舍）", () => {
		...
		expect(renumberContent("## 2024 总结")).toBe("## 1 总结");
	});

	it("用户在序号后补回数字：`1 2024 总结` 稳定保留（不被双吃）", () => {
		// 关键：空格不再被当作「段间序号间隔符」，且只剥一层 → `1 ` 被剥、`2024 总结` 当正文留下。
		expect(renumberContent("## 1 2024 总结")).toBe("## 1 2024 总结");
		// 反复触发幂等。
	...
	it("父子级同理：`1.1 2024 子` 只剥 `1.1 `、保留 `2024 子`", () => {
		// 段间用 `.` 连接的多段前缀作为一个单元被剥；其后空格隔开的 `2024` 是正文。
